...
L’entreprise « SuperCar4Everyone » malgré leur système de données logé dans le cloud, nous met à disposition la forme la plus évoluée de stockage de données : le CSV. Ce fichier contient un certain nombre de d’informations et un expert métier est là pour répondre à vos questions.
...
setwd(dirname(rstudioapi::getSourceEditorContext()$path))
datainput = read_delim('./data/trainTD3.csv', col_names = TRUE, delim = ',')
skim(datainput)
```

L' appréhension de la data via la fonction ‘skim’ est déjà un bon début car il nous donne l’idée de la population des contrats à étudier `r nrow(datainput)` et de l'interprétation des variables en 'character' ou 'numeric'. Elle nous indique les valeurs manquantes et le poid de ces 'n_missing'.

Le format de certaines variables nécessite d’être retravaillé, une rapide vue des variable permet de se faire une première idée :
...
Une première séléction des variables peut se faire via un échange avec les banquiers, après tout, ils connaissent mieux leur métier que nous! Il nous orientent sur les données qu'ils suivent pour mesurer le niveau de risque d'un client. Il sera utile de retenir a minima ces données et de les enrichir de variables que nous identifierons statistiquement.

Les étudiant doivent maintenant lire les descriptions des variables présentes, regarder les distributions et les premières valeurs et poser des questions aux métiers pour une près sélection des variables et une étape de wrangling.
...
dataWrangled <- dataWrangled %>%
  mutate(AvrLoantenure = as.integer((as.numeric(str_match_all( datainput$AVERAGE.ACCT.AGE,"\\d+(?=yrs)")) *12 
         + as.numeric(str_match_all( datainput$AVERAGE.ACCT.AGE,"\\d+(?=mon)"))))) %>%
  mutate(TimeSincefirstloan = (as.numeric(str_match_all( datainput$CREDIT.HISTORY.LENGTH,"\\d+(?=yrs)")) *12 
...
Il est à noter que la discrétisation d'une variable numérique constitue une perte d'information et n'est pas un pré-requis à la regression logistique. Néanmoins, la création de modalités permet souvent une lecture plus simple et une catégorisation des clients.
...
Nous allons utiliser la fonction woebin du package scorecard. Il découpe sur la base de la mesure WOE ou Weigth of Evidence. Le package fournit également une fonction woebin_plot qui propose des graphiques lisibles des découpages.
...
bins = woebin(dataWrangled[, c(to_bin,'loan_default')], y = 'loan_default',check_cate_num = F, bin_num_limit = 4)
...
woebin_plot(bins$AvrLoantenure)$AvrLoantenure
...
La matrice n'est pas très lisibile car elle embarque toutes les variables. C'est ici que nous devons faire une selection.

Methodologie proposee : on identifie les couples de variable correllees a plus de 30%. Au sein de chaque couple, on ne garde que celle qui est la plus liee au defaut.
...
Des variables d'identifiant ont été séléctionnées : branch_id_bin, supplier_id_bin, manufacturer_id_bin, State_ID_bin.
...
Une fois nos données préparées et nos variables séléctionnées, nous pouvons ajuster une régression logistique (modèle classiquement utilisé).
...
Remarque : des coefficients de la regression logistique ne sont pas significativement non nuls. Dans cette situation, il faudrait soit exclure la variable, soit revoir sa discrétisation jusqu'à avoir des coefficients tous significatifs
...
```
Comme attendu, les scores des clients tombant en défaut sont distribués dans les valeurs plus élevées. En effet, la régression logistique ajuste la probabilité d'avoir loan_default=1 en fonction des variables séléctionnées.
...
```{r quant1, echo=FALSE,  message=FALSE, warning=FALSE, layout="l-body-outset"}
setwd(dirname(rstudioapi::getSourceEditorContext()$path))
...
```{r quant2, echo=FALSE,  message=FALSE, warning=FALSE, layout="l-body-outset",class.source = 'fold-hide'}
...
```{r quant3, echo=FALSE,  message=FALSE, warning=FALSE, layout="l-body-outset",class.source = 'fold-hide'}
...
```{r quant4, echo=FALSE,  message=FALSE, warning=FALSE, layout="l-body-outset",class.source = 'fold-hide'}
...
```{r quant5, echo=FALSE,  message=FALSE, warning=FALSE, layout="l-body-outset",class.source = 'fold-hide'}
...
```{r quant6, echo=FALSE,  message=FALSE, warning=FALSE, layout="l-body-outset"}
...
```{r quant7, echo=FALSE,  message=FALSE, warning=FALSE, layout="l-body-outset",class.source = 'fold-hide'}
...
```{r quant9, echo=FALSE,  message=FALSE, warning=FALSE, layout="l-body-outset",class.source = 'fold-hide'}
...
```{r quant8, echo=FALSE,  message=FALSE, warning=FALSE, layout="l-body-outset"}
...
```{r quant10, echo=FALSE,  message=FALSE, warning=FALSE, layout="l-body-outset",class.source = 'fold-hide'}
